...
// If there only destroy instances for a particular resource, there's no reason
// for the resource node to prepare the state. Remove Resource nodes so that
// they don't fail by trying to evaluate a resource that is only being
